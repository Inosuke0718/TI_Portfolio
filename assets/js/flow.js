...
var mParticleCount = 10000; // 流れるParticleの数を変更できる
var mParticleSystem;
...
function initTHREE() {
  ...
  mCamera.position.set(160, -10, 160);
...
//   mContainer = document.getElementById('three-container2');
//   mContainer.appendChild(mRenderer.domElement);
...
function initParticleSystem() {
  ...
  for (i = 0, offset = 0; i < mParticleCount; i++) {
    axis.x = THREE.Math.randFloatSpread(10);
    axis.y = THREE.Math.randFloatSpread(2);
    axis.z = THREE.Math.randFloatSpread(10);
    axis.normalize();
...
THREE.BAS.PhongAnimationMaterial.prototype._concatVertexShader = function() {
  ...
  // function render(){
  //   requestAnimationFrame(render);// タブがActiveじゃない時は動作を抑制してくれる便利！
  //   mCamera = new THREE.PerspectiveCamera(60, window.innerWidth / window.innerHeight, 0.1, 5000);
  //   mScene = new THREE.Scene();
  //   mRenderer = new THREE.WebGLRenderer({antialias: true});

  //   theta += 0.1;
  //   // mCamera.position.x = Math.cos(THREE.Math.degToRad(theta)) * 300;
  //   mCamera.position.y = Math.sin(THREE.Math.degToRad(theta)) * 300;
